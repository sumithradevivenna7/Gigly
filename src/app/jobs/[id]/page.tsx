...
export default function JobDetailPage() {
  ...
  const [estimatedDuration, setEstimatedDuration] = useState('3 weeks');
  ...
  const handleProposalSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setProposalSuccess('Your proposal has been submitted to the client! You will be notified when reviewed.');
    } catch (err: unknown) {
      const e = err as Error;
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="glass-panel max-w-md w-full p-8 rounded-3xl border border-emerald-500/40 text-center space-y-5 shadow-2xl">
            ...
            <div className="flex items-center justify-center gap-3 pt-2">
              ...
              >
                View Contract & Milestone
              </Link>
              <button
                onClick={() => setEscrowSuccessModal(null)}
                className="bg-surface-800 text-surface-300 hover:text-white px-4 py-2.5 rounded-xl text-xs"
              >
                Close
              </button>
            </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-surface-800 space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 p-4 rounded-2xl bg-surface-900/80 border border-surface-800">
              <div>
                <span className="text-[11px] text-surface-400">Budget</span>
              ...
              <div>
                <span className="text-[11px] text-surface-400">Project Duration</span>
              ...
              <div>
                <span className="text-[11px] text-surface-400">Deadline</span>
                <div className="text-xs font-bold text-white mt-1 flex items-center gap-1">
          ...
          {(!isClientOwner || activeTab === 'description') && (
            <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-surface-800 space-y-6">
              <div>
                <h3 className="text-base font-bold text-white mb-3">Project Specifications & Scope</h3>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-base font-bold text-white">
                  Freelancer Bids ({proposals.length})
                </h3>
                <span className="text-xs text-surface-400">
                  Review cover letters and fund escrow on hire
                </span>
              </div>
              ...
              <div className="flex items-center justify-between pb-4 border-b border-surface-800">
                <div>
                  ...
                  <p className="text-xs text-surface-400 mt-1">
                    Explain your architectural strategy, milestones, and price bid.
                  </p>
                </div>
                <div className="text-right">
                  <span className="text-[11px] text-surface-400">Total Bids</span>
                  <div className="text-sm font-bold text-white">{proposals.length} submitted</div>
                </div>
              </div>
              ...
              {existingProposal ? (
                <div className="p-5 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 space-y-3">
                  <div className="flex items-center gap-2 text-emerald-400 text-sm font-bold">
                    ...
                    <span>You already applied for this position!</span>
                  </div>
                  <p className="text-xs text-surface-300">
                    Your bid of <strong>${existingProposal.bidAmount}</strong> was received. Current status:{' '}
                ...
                <form onSubmit={handleProposalSubmit} className="space-y-6">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-surface-300 mb-1.5">
                      Cover Letter & Proposed Milestones
                    </label>
                    <textarea
                      rows={6}
                      placeholder="Outline how you will solve the client's problem, your technical stack decisions, and breakdown of milestones..."
                      value={coverLetter}
